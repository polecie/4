...
class BaseEmailValidation(ABC):
    ...
    def check_domain(email: str, domain: str) -> bool:
        """
        Абстрактный метод.
        Проверка наличия выбранного пользователем домена, записанного в состояние,
        в электронном адресе почты, указанным пользователем для привязки.
        Например, проверка на наличие домена @gmail.com или @yandex.ru в `user_email`.
        :param email: Электронный адрес, указанный пользователем для привязки.
